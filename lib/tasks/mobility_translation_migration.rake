...
namespace :translations do
  namespace :mobility do
    ...
    task migrate_names_to_string: :environment do
      puts 'Starting migration of community/partner names from text to string translations...'
      puts '=' * 80
      ...
      text_name_translations = text_translation_class.where(
        translatable_type: ['BetterTogether::Community', 'Partner'],
        key: 'name'
      )

      # Also find any community subclasses that might have been stored incorrectly
      community_subclass_translations = text_translation_class.where(
        key: 'name'
      ).where('translatable_type LIKE ?', '%Community%')

      all_text_translations = (text_name_translations + community_subclass_translations).uniq
    ...
    task check_names_status: :environment do
      ...
      community_subclass_string_translations = string_translation_class.where(
        key: 'name'
      ).where('translatable_type LIKE ?', '%Community%')

      all_string_translations = (string_name_translations + community_subclass_string_translations).uniq
